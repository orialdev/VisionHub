--[[
    Merged GalaxyUI com todas as funcionalidades do OrionLib
    Versão: Merged v3.0
--]]
...
-- Funções de Config (Salvar/Carregar)
...
-- #################################################################
-- ##           ElementLibrary – Criação de Elementos Básicos       ##
-- #################################################################
...
function ElementLibrary:Label(text, textSize, transparency)
    return Create("TextLabel", {
        Text = text or "",
        TextColor3 = GalaxyUI.Themes.Default.TextColor,
...
-- #################################################################
-- ##       Extensão dos ElementFunction – Elementos Avançados       ##
-- #################################################################

-- Para que os elementos sejam inseridos corretamente, definimos ItemParent
...
function ElementFunction:AddButton(ButtonConfig)
    ...
    local ButtonFrame = AddThemeObject(
        Create("Frame", {
            ...
            Parent = ItemParent,
            BackgroundColor3 = GalaxyUI.Themes.Default.Second
        }, {
            AddThemeObject(MakeElement("Label", ButtonConfig.Name, 15), "Text"),
            AddThemeObject(MakeElement("Image", ButtonConfig.Icon), "TextDark"),
    ...
    AddConnection(Click.MouseEnter, function()
        TweenService:Create(ButtonFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint), 
            {BackgroundColor3 = GalaxyUI.Themes.Default.Second + Color3.new(0.01,0.01,0.01)}):Play()
    end)
    AddConnection(Click.MouseLeave, function()
        TweenService:Create(ButtonFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint), 
            {BackgroundColor3 = GalaxyUI.Themes.Default.Second}):Play()
    end)
    AddConnection(Click.MouseButton1Down, function()
        TweenService:Create(ButtonFrame, TweenInfo.new(0.25, Enum.EasingStyle.Quint), 
            {BackgroundColor3 = GalaxyUI.Themes.Default.Second + Color3.new(0.02,0.02,0.02)}):Play()
    end)
...
function ElementFunction:AddToggle(ToggleConfig)
    ...
    local ToggleBox = Create("Frame", {
        ...
        BackgroundColor3 = Toggle.Value and ToggleConfig.Color or GalaxyUI.Themes.Default.Divider
    }, {
        AddThemeObject(MakeElement("Stroke"), "Stroke"),
        AddThemeObject(MakeElement("Image", "rbxassetid://3944680095"), "Text")
    })
    local ToggleFrame = AddThemeObject(
        Create("Frame", {
            ...
            AddThemeObject(MakeElement("Label", ToggleConfig.Name, 15), "Text"),
            AddThemeObject(MakeElement("Stroke"), "Stroke"),
            ToggleBox,
            Click
        }),
    ...
    function Toggle:Set(Value)
        ...
        TweenService:Create(ToggleBox, TweenInfo.new(0.3, Enum.EasingStyle.Quint), 
            {BackgroundColor3 = Toggle.Value and ToggleConfig.Color or GalaxyUI.Themes.Default.Divider}):Play()
    ...
    if ToggleConfig.Flag then
        GalaxyUI.Flags[ToggleConfig.Flag] = Toggle
    end
    return Toggle
end
...
function ElementFunction:AddSlider(SliderConfig)
    ...
    local Dragging = false
    local SliderDrag = Create("Frame", {Size = UDim2.new(0, 0, 1, 0), BackgroundColor3 = SliderConfig.Color, BackgroundTransparency = 0.3}, {
        AddThemeObject(MakeElement("Label", "value", 13), "Text")
    })
    local SliderBar = Create("Frame", {
        ...
        AddThemeObject(MakeElement("Stroke"), "Stroke"),
        Create("TextLabel", {Name = "Value", Text = "value", Font = Enum.Font.GothamBold}, nil),
        SliderDrag
    })
    local SliderFrame = AddThemeObject(
        Create("Frame", {
            ...
            AddThemeObject(MakeElement("Label", SliderConfig.Name, 15), "Text"),
            AddThemeObject(MakeElement("Stroke"), "Stroke"),
            SliderBar
        }),
    "Second")
    ...
    if SliderConfig.Flag then
        GalaxyUI.Flags[SliderConfig.Flag] = Slider
    end
    return Slider
end
...
function ElementFunction:AddDropdown(DropdownConfig)
    ...
    local DropdownFrame = AddThemeObject(
        Create("Frame", {
            ...
            DropdownContainer,
            Create("Frame", {Name = "F", Size = UDim2.new(1, 0, 0, 38), ClipsDescendants = true}, {
                AddThemeObject(MakeElement("Label", DropdownConfig.Name, 15), "Text"),
                AddThemeObject(MakeElement("Image", "rbxassetid://7072706796"), "TextDark"),
                AddThemeObject(MakeElement("Label", "Selected", 13), "TextDark"),
        ...
        for _, Option in pairs(Options) do
            local OptionBtn = AddThemeObject(
                Create("TextButton", {
                    ...
                    AddThemeObject(MakeElement("Label", Option, 13, 0.4), "Text")
                }),
            "Divider")
    ...
    if DropdownConfig.Flag then				
        GalaxyUI.Flags[DropdownConfig.Flag] = Dropdown
    end
    return Dropdown
end
...
function ElementFunction:AddBind(BindConfig)
    ...
    local BindBox = AddThemeObject(
        Create("Frame", {
            ...
            AddThemeObject(MakeElement("Stroke"), "Stroke"),
            AddThemeObject(MakeElement("Label", BindConfig.Name, 14), "Text")
        }),
    "Main")
    local BindFrame = AddThemeObject(
        Create("Frame", {
            ...
            AddThemeObject(MakeElement("Label", BindConfig.Name, 15), "Text"),
            AddThemeObject(MakeElement("Stroke"), "Stroke"),
            BindBox,
            Click
        }),
    ...
    if BindConfig.Flag then				
        GalaxyUI.Flags[BindConfig.Flag] = Bind
    end
    return Bind
end
...
function ElementFunction:AddTextbox(TextboxConfig)
    ...
    local TextboxFrame = AddThemeObject(
        Create("Frame", {
            ...
            AddThemeObject(MakeElement("Label", TextboxConfig.Name, 15), "Text"),
            AddThemeObject(MakeElement("Stroke"), "Stroke"),
            TextContainer,
            Click
        }),
...
function ElementFunction:AddColorpicker(ColorpickerConfig)
    ...
    local ColorpickerFrame = AddThemeObject(
        Create("Frame", {
            ...
            Create("Frame", {Name = "F", Size = UDim2.new(1, 0, 0, 38), ClipsDescendants = true}, {
                AddThemeObject(MakeElement("Label", ColorpickerConfig.Name, 15), "Text"),
                ColorpickerBox,
                Click,
                AddThemeObject(MakeElement("Frame"), "Stroke")
            }),
    ...
    if ColorpickerConfig.Flag then				
        GalaxyUI.Flags[ColorpickerConfig.Flag] = Colorpicker
    end
    return Colorpicker
end

-- Função para adicionar uma seção (ex.: para agrupar elementos)
function ElementFunction:AddSection(SectionConfig)
    SectionConfig.Name = SectionConfig.Name or "Section"
    local SectionFrame = Create("Frame", {
        Size = UDim2.new(1, 0, 0, 26),
        Parent = Container  -- Container aqui é o mesmo que ItemParent do Tab atual
    }, {
...
-- Aqui, copiamos os métodos do container para o ElementFunction final
for i, v in next, Container:GetChildren() do
    ElementFunction[i] = v 
end

-- Se for premium, bloqueia os elementos
-- (Essa parte é opcional e pode ser adaptada conforme sua lógica de premium)
if false then  -- Exemplo: se TabConfig.PremiumOnly for true
    ...
    Create("Frame", {
        ...
        AddThemeObject(Create("TextLabel", {
            ...
            Text = "This part of the script is locked to Sirius Premium users. Purchase Premium in the Discord server (discord.gg/sirius)",
            TextWrapped = true,
            TextTransparency = 0.4,
            Font = Enum.Font.Gotham
        }), "Text")
...
-- Retorna a tabela com todos os métodos do ElementFunction
-- (Essa função encapsula os métodos para criação de elementos)
local function GetElementFunctions(Container)
    return ElementFunction
end

-- Exemplo de notificação (como no OrionLib)
function GalaxyUI:Notify(config)
    ...
    local frame = Create("Frame", {
        ...
        BackgroundColor3 = GalaxyUI.Themes.Default.Topbar,
        Parent = sg
    })
...
-- #################################################################
-- ##              Criação de Janela e Abas (Exemplo)              ##
-- #################################################################
...
function GalaxyUI:CreateWindow(opt)
    ...
    local main = Create("Frame", {
        Name = "MainFrame",
        Size = UDim2.new(0, 780, 0, 450),
        Position = UDim2.new(0.5, -390, 0.5, -225),
        BackgroundColor3 = GalaxyUI.Themes.Default.MainFrame,
        BackgroundTransparency = 1,
        Parent = screenGui
    })
    ...
    local side = Create("Frame", {
        Name = "Sidebar",
        Size = UDim2.new(0, GalaxyUI.Themes.Default.SidebarWidth, 1, -45),
        Position = UDim2.new(0, 0, 0, 45),
        BackgroundColor3 = GalaxyUI.Themes.Default.SidebarColor,
        Parent = main
    })
    local container = Create("Frame", {
        Name = "Container",
        Size = UDim2.new(1, -GalaxyUI.Themes.Default.SidebarWidth, 1, -45),
        Position = UDim2.new(0, GalaxyUI.Themes.Default.SidebarWidth, 0, 45),
        BackgroundTransparency = 1,
        Parent = main
    })
...
local window = GalaxyUI:CreateWindow({Name = "Exemplo Merged GalaxyUI", Theme = "Default"})
local tabGeneral    = window:CreateTab({Name = "Geral", Icon = "rbxassetid://4483362458"})
local tabSettings   = window:CreateTab({Name = "Configurações", Icon = "rbxassetid://4483362458"})
local tabAdvanced   = window:CreateTab({Name = "Avançado", Icon = "rbxassetid://4483362458"})

-- Aba Geral
do
    ItemParent = tabGeneral.Frame
    local welcomeSection = Create("Frame", {
        Size = UDim2.new(1, -20, 0, 40),
        BackgroundColor3 = GalaxyUI.Themes.Default.SectionColor,
        Parent = tabGeneral.Frame
    })
    local welcomeLabel = Create("TextLabel", {
        Size = UDim2.new(1, -20, 1, 0),
        Position = UDim2.new(0, 10, 0, 0),
        Text = "Olá, bem-vindo ao GalaxyUI Merged!",
        TextColor3 = GalaxyUI.Themes.Default.TextColor,
        Font = Enum.Font.GothamSemibold,
        TextSize = 18,
        BackgroundTransparency = 1,
    ...
    local notifyButton = Create("TextButton", {
        Size = UDim2.new(1, -20, 0, 40),
        Position = UDim2.new(0, 10, 0, 50),
        BackgroundColor3 = GalaxyUI.Themes.Default.ButtonColor,
        Text = "Clique aqui para notificar",
        TextColor3 = GalaxyUI.Themes.Default.TextColor,
        Font = Enum.Font.Gotham,
        TextSize = 16,
        Parent = tabGeneral.Frame
    ...
    notifyButton.MouseButton1Click:Connect(function()
        GalaxyUI:Notify({Title = "Aviso", Content = "Você clicou no botão!", Duration = 3})
    end)
end

-- Aba Configurações
do
    ItemParent = tabSettings.Frame
    local prefSection = Create("Frame", {
        Size = UDim2.new(1, -20, 0, 40),
        BackgroundColor3 = GalaxyUI.Themes.Default.SectionColor,
        Parent = tabSettings.Frame
    })
    local featureToggle = ElementFunction:AddToggle({
        Name = "Ativar Recurso",
        Default = false,
        Callback = function(val) print("Recurso ativado:", val) end,
        Flag = "FeatureEnabled",
        Save = true
    })
end
...
-- Aba Avançado
do
    ItemParent = tabAdvanced.Frame
    local hotkeyBind = ElementFunction:AddBind({
        Name = "Atalho",
        Default = Enum.KeyCode.None,
        Hold = false,
        Callback = function(hold) print("Atalho acionado, hold:", hold) end,
        Flag = "Hotkey",
        Save = true
    })
    local inputBox = ElementFunction:AddTextbox({
        Name = "Digite algo",
        Default = "",
        TextDisappear = false,
        Callback = function(text) print("Texto inserido:", text) end
    })
    local modeDropdown = ElementFunction:AddDropdown({
        Name = "Selecione o Modo",
        Options = {"Fácil", "Médio", "Difícil"},
        Default = "Médio",
        Callback = function(option) print("Modo selecionado:", option) end,
        Flag = "GameMode",
        Save = true
    })
    local volumeSlider = ElementFunction:AddSlider({
        Name = "Volume",
        Min = 0,
        Max = 100,
        Increment = 1,
        Default = 50,
        Callback = function(val) print("Volume definido para:", val) end,
        ValueName = "%",
        Flag = "VolumeLevel",
        Save = true
    })
    local colorPicker = ElementFunction:AddColorpicker({
...
GalaxyUI:EnableConfig(true, "GalaxyConfigs", "MeuConfig")
GalaxyUI:LoadConfig()
GalaxyUI:Notify({Title = "GalaxyUI Merged", Content = "A interface mesclada foi iniciada com sucesso!", Duration = 5})
