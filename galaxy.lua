...
local function saveConfig()
	if not GalaxyUI.Config.Enabled then
		return
	end
	local data = {}
...
function GalaxyUI:CreateWindow(opt)
	...
	minimize.MouseButton1Click:Connect(function()
		if tick() - w.lastToggleTime < 2 then
			return
		end
...
function GalaxyUI:CreateColorPicker(opt)
	...
	label.Position = UDim2.new(0, 10, 0, 0)
	label.BackgroundTransparency = 1
	label.Text = opt.Name or "Color Picker"
	label.TextColor3 = GalaxyUI.Themes.Default.TextColor
	label.Font = Enum.Font.Gotham
	label.TextSize = 16
	label.TextXAlignment = Enum.TextXAlignment.Left
	...
	colorDisplay.BackgroundColor3 = opt.DefaultColor or Color3.new(1, 1, 1)
	colorDisplay.Parent = container
	local cdCorner = Instance.new("UICorner", colorDisplay)
	...
	pickerFrame.BackgroundColor3 = GalaxyUI.Themes.Default.ButtonColor
	...
	local grid = Instance.new("UIGridLayout", pickerFrame)
	...
	for _, clr in ipairs(colors) do
		...
		btn.MouseButton1Click:Connect(function()
			colorDisplay.BackgroundColor3 = clr
			if opt.Callback then
				opt.Callback(clr)
			end
			pickerFrame.Visible = false
...
function GalaxyUI:CreateMultiDropdown(opt)
	...
	local function updateMainButton()
		...
		if txt == "" then
			txt = opt.Name or "Select Options"
		end
		mainButton.Text = txt
	end
	local function populate(list)
	...
	return {Frame = container, Main = mainButton, SetOptions = function(self, list)
		optionsList = list
		populate(optionsList)
	end}
end
function GalaxyUI:EnableResizing(window)
	local resizer = Instance.new("TextButton", window.Main)
	resizer.Size = UDim2.new(0, 20, 0, 20)
	resizer.Position = UDim2.new(1, -20, 1, -20)
	resizer.BackgroundColor3 = GalaxyUI.Themes.Default.AccentColor
	resizer.Text = ""
	local rsCorner = Instance.new("UICorner", resizer)
	...
	resizer.InputChanged:Connect(function(input)
		if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
			local delta = input.Position - dragStart
			window.Main.Size = UDim2.new(startSize.X.Scale, startSize.X.Offset + delta.X, startSize.Y.Scale, startSize.Y.Offset + delta.Y)
